...
当前 applet 已实现共享的 CPU、内存、hwmon 温度和风扇异步采样，实例级右键编排、状态颜色、稳定宽度、完整工具提示，以及各指标的左键详情页。温度和风扇仅在至少一个可见布局项使用时采集，支持主要值、峰值和平均值格式；温度详情列出所有传感器及临界值，风扇详情列出全部转速。开发验证时可以从右键菜单添加“状态演示（开发）”，循环检查正常、警告、危险、不可用和等待状态；它默认不会出现在面板中，也不会访问硬件。
