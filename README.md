...
当前 applet 已实现共享的 CPU / 内存异步采样、实例级右键编排、状态颜色、稳定宽度、完整工具提示，以及 CPU / 内存左键详情页。开发验证时可以从右键菜单添加“状态演示（开发）”，循环检查正常、警告、危险、不可用和等待状态，并切换百分比、温度、字节、速率和 RPM 格式；它默认不会出现在面板中，也不会访问硬件。
